...
let wordsList = function (str, subStr) {
  let array = str
    ...
    .toLowerCase()
    .split(' ')
    .filter((word) => word.includes(subStr));
...
function getLocalDate(date, isSeconds, isISO) {
  ...
  const datewithoutSeconds = `${day}.${month}.${year}, ${hour}:${minutes}`;
  const datewithoutSecondsISO = `${year}-${month}-${day}, ${hour}:${minutes}`;
  let res;
  ...
  if (isISO && !isSeconds) {
    res = datewithoutSecondsISO;
  }

  if (!isISO && !isSeconds) {
    res = datewithoutSeconds;
  }
  return res
}
